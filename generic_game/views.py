...
def get_game_context(request, **kwargs):
  ...
  game_masters = GameUser.objects.filter(game=game).exclude(gamemaster=None)
  characters = GameUser.objects.filter(game=game).exclude(character=None)
  context['game'] = game
  ...
  if request.user.is_authenticated():
    try:
      if "gm" in kwargs:
        ...
        gm = game_masters.get(gamemaster=request.user)
      context['gm'] = gm
